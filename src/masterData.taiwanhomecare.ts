import type { Tenant } from "./classify.js";

// 模擬 Ragic 主檔資料（台灣福祉科技，福祉車／復康巴士改裝廠）。正式版由 Ragic API 拉取。
// 人名皆為假名（姓＋○○），供 demo／回歸使用。
export const taiwanHomecareMasterData = {
  ...
  // 福祉車詞庫：口語／台語 → 標準語意。每家改裝廠可累積自己的詞庫。
  glossary: {
    "尾門機／升降機": "輪椅升降機（升降平台）",
    "坡板／斜坡": "斜坡板",
    "綁帶／固定帶": "輪椅安全固定帶",
...
const masterDataJson = JSON.stringify(taiwanHomecareMasterData, null, 2);
...
export const TAIWANHOMECARE_TENANT: Tenant = { systemPrompt: SYSTEM_PROMPT, masterDataJson };
