...
> 「看起來像 AI 生成樣板 / 千篇一律」是這條的**失敗態**。但**問題出在「通用執行」、不是特定配色**——**不預先禁任何配色/風格**（奶油、深色、報紙、任何色系，只要做得好、貼題、有理由都可以）。不用配色黑名單判斷 AI 感，改用「**刻意＞出廠預設 ＋ 貼題 ＋ 有記憶點**」；把大膽花在能說出理由的地方（見 §C4）。

**先鎖具體對標（動手前必做，2026-07-03 用戶裁定新增）**：設計前先鎖**一個真實參照物**——你欣賞的真實產品 URL／截圖，或「做得像 ⟨具名產品/品牌⟩」。**照著一個具體目標「match」，遠勝過從「模型的平均值」generate**（後者正是 AI 感的根源）。沒有對標就先要一個，再進 §A5 研究與 §C 流程。
> 註：舊版此處列的「三組 AI 預設群集（奶油+襯線+赤土／深底+螢光／報紙格線）」黑名單已刪除（2026-07-03）——那些方向做得好都成立，禁它們是鈍器。
