...
> **客戶裁定（2026-07-24 · 洪鈺仙）**：「不用（在戰情室）查看，就是**透過 API 回寫給 Ragic**，我會開一個**費用支出**表，然後**像訂單一樣讓他確認就好，一模一樣**。」
> → 本功能**不需戰情室檢視 UI**；核心是 拍照 → AI 抽取 → 回寫 Ragic 費用支出表 → 比照既有「訂單」確認流程。
...
## 7. 開放問題（OQ · 待裁定才進 M1）
- ~~OQ-EXP-1 Ragic 目標表~~ → **客戶裁定（2026-07-24）**：客戶方**自行在 Ragic 開一張「費用支出」表**，本功能**透過 API 回寫**該表（沿用 `POST …?api` URLEncoded pattern）。**欄位對應待那張表定案後由客戶提供**。發票照片是否附 Ragic：TBD（Ragic 支援附檔）。
...
- ~~OQ-EXP-6 簽核流程~~ → **客戶裁定**：「**像訂單一樣讓他確認就好，一模一樣**」→ 沿用既有「**訂單**」的確認 pattern（拍照 → AI 抽取 → 回寫 Ragic → 比照訂單確認）。客戶並說「**不用（在戰情室平台）查看**」→ 本功能**不需另做戰情室檢視 UI**，確認比照訂單流程。
- **OQ-EXP-6b 訂單確認流程**（殘留 · M1 需釐清）：既有「訂單」是**在哪裡確認、怎麼確認**（Ragic workflow / Ragic 表內確認欄位 / 戰情室簽核）？要對齊才能做「一模一樣」。
