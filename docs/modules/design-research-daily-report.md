...
> 狀態：✅ **M0 定案 · M1 前端完成 = C 表格 + A 日期分組（hybrid）**（2026-08-01）
> 演進：C（精簡表格）→ A（分組時間軸）→ 最終**回到 C 的表格、搭配 A 的日期分組**＝分組表格：
> `.dm-table` 列 + 每個日期一條 `.dr-grp-row` 群組列（可收合）。狀態 `.st-dot`+`.nc-pill`、`.seg` 過濾沿用。
...
| # | 問題 | 裁定 |
|---|---|---|
| ~~**OQ-DR-1**~~ | 三個版面方向選哪個？ | ✅ **最終＝C 表格 + A 日期分組（hybrid）**· 演進 C→A→C+A · B mockup 留檔 |
...
| **M1** ✅ | 重構 `TeamDailyReport.tsx`（前端 only）· **方向 A 分組時間軸**：日期分組（date·星期·N 筆·可收合）+ 時間軸列（●燈點/姓名/部門/pill/項數/首事項預覽/展開）· 預設過濾空列 + `有內容/未回報/全部` segmented(`.seg`) + 部門 StyledSelect + 底部缺席聚合 + §A6 五態 · 新增 `.dr-*` class + 複用 `.st-dot`/`.nc-pill`/`.seg`/`.dm-info-note`/`.pdr-item` · §A10 對 mockup A 並排比對截圖驗 · web tsc 綠 |
