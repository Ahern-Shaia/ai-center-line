...
> 狀態：✅ **M0 定案 · M1 前端完成 = 方向 A 分組時間軸**（2026-08-01）
> 註：先做 C（精簡表格）上線，同日用戶改要 **A（分組時間軸 · 對標 Linear）**，已切換實作。C 版樣式（`.seg`/`.st-dot`/`.nc-pill`）沿用，改的是主結構＝日期分組 + 時間軸列。
...
| # | 問題 | 裁定 |
|---|---|---|
| ~~**OQ-DR-1**~~ | 三個版面方向選哪個？ | ✅ **最終＝A 分組時間軸**（對標 Linear）· 先上 C（精簡表格）同日改 A · B mockup 留檔 |
