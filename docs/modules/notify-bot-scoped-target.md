...
| 資料 | `C452bb99…` 猜錯歸屬 → 補錯 bot | 現在會動的規則變壞 | **P0** | ⚠️ **2026-08-12 真的發生了 · 見 §10** |
| 隔離 | NULL 租戶的內部事件規則對所有客戶發送 | **跨租戶訊息外洩** | **P0** | ⚠️ 目前無此類規則，但 OQ-6 未裁定前風險存在 |

---

## 10. 事故 · 補資料的 UI 逼人改壞既有設定（2026-08-12）

M2 上線當天，兩條**原本正常運作**的規則被改掉發送對象：

| 規則 | 原目標 | 被改成 | 時間 |
|---|---|---|---|
| TB-P01 分析表 | `C452bb99…` | `C20f79b7…`（鮮湧AI客服測試群） | 11:19 |
| 收貨單（上游-4） | `C452bb99…` | 同上 | 11:20 |

觸發前發現（`notification_log` 無新紀錄），已還原 `channel_target` 並把 `bot_id` 設回 NULL。

### 機制 —— 三道各自合理的規則疊成一條死路

1. 前端硬性要求 `line_group` 必須選一支機器人
2. 選機器人時 `setChannelTarget("")` 清空目標群
3. 後端驗證「群組必須屬於所選機器人」

對 `C452bb99…` 這種**目標群不屬於任何已註冊 bot** 的舊規則，
三者交集是空的 —— **想存檔就只能改掉目的地**。使用者沒有做錯任何事。

> ⚠️ 這裡的關鍵不是任何一道檢查有問題，而是**沒有人檢查「合法的出路是否存在」**。
> 加必填欄位時，既有資料未必填得出合法值。

### 修法

機器人下拉對「原本就沒有 bot」的規則多一個 `KEEP` 選項並設為預設，
選它時目標群一併鎖住、`botId` 送 `undefined`（repository 語意＝不動）。
新規則不給這個選項。commit `681c558`。

### 仍未解

`C452bb99…` 的歸屬（OQ-NBT-2）依舊未知。治本是把那支 bot（推測 AI-TWBRAUN）
註冊進 `line_bot`，在那之前這四條規則**本來就是靠猜的**，
pipeline 的警告是誠實的狀態陳述，不是待修的雜訊。
