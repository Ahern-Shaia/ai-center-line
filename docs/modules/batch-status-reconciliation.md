...
> 狀態：📋 **M0 DRAFT v0.1**（2026-07-30）· 待用戶裁定 OQ-BSR-1..7
>
...
| **M1** | C · 對帳查詢 + 掛上抽取健康度頁（純讀，先拿到能見度）| OQ-BSR-4/5 |
| **M2** | A · 下游回寫 + `scheduleJob`/`runJob` 足跡 log | OQ-BSR-1/2 |
...
| **OQ-BSR-5** | 對帳結果給誰看？ | **aiproot 看全部、tenant_admin 看自家**。依 `feedback_tenant_self_governance`，客戶方會想自己看得到「昨天有沒有漏」 |
...
| 失效模式 | 影響 | 嚴重度 | 緩解 |
|---|---|---|---|
| 回寫本身被 RLS 靜默擋掉（回 0 列不報錯）| **裝了儀表但它永遠顯示正常** —— 比沒裝更糟，因為會讓人停止懷疑 | **P0** | 🔒 待 M2：先確認 `analysis_batch` 的 policy，並加測試斷言「回寫後真的讀到 failed」 |
| 手動上傳（無 batch）也走回寫 → 更新不存在的列 | 無害（0 列）但若寫成 throw 會讓成功的上傳變失敗 | P1 | 🔒 待 M2：回寫前先確認有 batch，且回寫失敗不得影響 upload 主流程 |
| 前端遇到新的 status 值顯示空白 | 又一個「失敗長得像正常」 | P1 | 🔒 待 M2：OQ-BSR-1 若採「沿用 `failed`」則不存在 |

### 11.2 C · 對帳查詢

| 失效模式 | 影響 | 嚴重度 | 緩解 |
|---|---|---|---|
| 查詢漏設 RLS 變數 → 回 0 列 → 顯示「一切正常」 | **P0 · 同上，儀表騙人** | **P0** | 🔒 待 M1：走既有 `currentTx()`（繼承 interceptor 上下文）而非自己開 tx；加測試「塞一筆分歧資料必須被列出」 |
| N 分鐘門檻太短 → 把正在跑的報成失敗 | 狼來了，人開始忽略這個儀表 | P1 | ✅ OQ-BSR-4 建議 30 分鐘（單群實測數十秒到數分鐘）|
| 跨租戶洩漏（tenant_admin 看到別家失敗數）| 隱私 | P1 | 🔒 待 M1：沿用 `current_tenant`，加跨租戶隔離測試 |
