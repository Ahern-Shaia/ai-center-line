...
> 狀態：📋 **M0 APPROVED v0.2**（2026-07-31）· ⭐ 架構已定：**aiproot 獨立通用 bot（只回 ID）· 與租戶 bot 分開** · 待裁定 OQ-GID-2/3
>
...
| # | 內容 | 依賴 |
|---|---|---|
| **M0** 📋 | 本文件 · 待裁定 OQ-GID-2/3 ← 在這 | — |
| **M1** | 通用 bot channel 註冊 + 獨立極簡 handler · join 歡迎附 ID + 關鍵字回 ID · 不落庫 | OQ-GID-2 |
...
| **OQ-GID-2** | 用哪個關鍵字？`群組ID` / `#id` / `群組id`？ | 要好記、又不易在正常對話誤觸 · 通用 bot 只做這件事，關鍵字可以取得更直白 | 待裁定 |
| **OQ-GID-3** | 加群歡迎訊息直接附上 ID，還是只在打關鍵字才給？ | 直接附＝零操作但一定發言；只在關鍵字給＝更安靜。通用 bot 情境下「加進去就是為了拿 ID」，傾向直接附 | 待裁定 |
| **OQ-GID-4** | 長期要不要讓客戶**完全不碰原始 ID**（notify 也改按名字選）？ | 若是，通用 bot 只是過渡；但它成本極低，留著當「進階/驗證」管道也無妨 | 待裁定 |
