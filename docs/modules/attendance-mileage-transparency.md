...
**外部依據（2026-07-26 補）**：競品「業務王 Salesking」同類外勤 APP 的 App Store 評分 **1.35★**、Google Play **2.2★**，一星評論集中在「為了省錢叫員工下載」「反應問題沒人理」——買方（老闆）與用方（員工）分裂是這個賽道的結構性風險。本模組「把依據攤給員工自己看」的方向因此被提升為產品鐵則：**每個對主管有價值的功能，都要同時對員工有一個看得到的價值**。詳見 [`../market/競品分析-業務王Salesking.md`](../market/競品分析-業務王Salesking.md) §5。
