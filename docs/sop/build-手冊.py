#!/usr/bin/env python3
"""把 docs/sop 的兩份 HTML 手冊合併成單一可部署檔 戰情室操作手冊.html

  python3 docs/sop/build-手冊.py

⚠️ 合併檔是**生成的**，不要手改它 —— 改內容請改兩份來源：
   ragic-line-通知設定-手冊.html / 權限設定-手冊.html，再重跑本腳本。

兩份來源的 CSS 有同名 class 衝突（.fld/.note/.step/table 定義不同），
本腳本把各自的選擇器加上 #g-ragic / #g-perm 前綴（:root/*/body 保留全域），
再包成分頁切換的單一頁。
"""
import re
import os
...
<style>
  ...
  *{{box-sizing:border-box}}
  ...
  #g-ragic .wrap, #g-perm .wrap{{max-width:820px;margin:0 auto;padding:32px 24px 80px}}
...
<div class="hub"><div class="hub-in">
  ...
  <button class="hub-tab on" data-doc="g-ragic">Ragic → LINE 通知設定</button>
  <button class="hub-tab" data-doc="g-perm">權限設定（總經理 / 部門主管）</button>
</div></div>

<section class="doc on" id="g-ragic"><div class="wrap">
{rag_body}
</div></section>
