...
> ✅ **2026-08-28：中英雙語背景圖已由用戶上傳至 manager.line.biz。**
...
| 3 | 智慧聯網戰情室 | ⛔ **刻意不做**（2026-08-28 裁定）| `shianyong-warroom.html` 維持全中文 —— **這頁是給中文語系客戶展示用的**，不是給外籍員工每天操作。<br>⚠️ 這是**唯一一格「入口有英文、落地頁刻意沒有」**。可接受的理由：選單上的 `Ops Command Center` 是**讓英文讀者認得出這顆按鈕**，不是承諾內容是英文；而前兩格（報工單／我的日報）是員工天天要用的，那兩格必須兌現。 |
| 4 | 內部客服機器人 | — | 未開放 |
...
>    否則同一個目錄一半英文一半中文）
> ② ⛔ `shianyong-warroom.html` 雙語化 —— **2026-08-28 裁定不做**。
>    那頁是給**中文語系客戶展示**用的，不是外籍員工的日常操作介面。
>    **判準：入口要不要兌現英文，看的是「那個落地頁誰在用」** ——
>    員工天天用的（報工單 / 我的日報）必須兌現；展示用的不必。
>    ~~建議跟視覺改版一起做~~ —— 那頁**同時排在視覺改版**
