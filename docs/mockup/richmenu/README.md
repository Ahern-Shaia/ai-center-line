...
| 鮮湧 | 鮮湧AI客服 | 2 欄 × 2 列 | 4 | `shianyong/`（僅大版，見下）|
...
| # | 標籤 | 現況 | 要接什麼 |
|--:|---|---|---|
| 1 | 報工單 | ⚠️ **沒有對應頁面** | LIFF 只認得 `mine` / `punch` / `trips`（`web/src/liff/main.tsx:100`）· 要嘛新增一個 page、要嘛先指到 Ragic 表單網址 |
| 2 | 我的日報 | ✅ 頁面已上線 | `?page=mine` |
| 3 | 智慧聯網戰情室 | ✅ 已上線 | 一般網頁，非 LIFF：`https://ai-center-line-demo.onrender.com/shianyong-warroom.html`（平面圖監控＋批次品質兩個分頁）· 用 URI 動作直接貼，不帶 `botId`／`liffId` |
| 4 | 內部客服機器人 | 🔒 **預寫、尚未開放** | 背景圖上刻意畫成**灰階＋「尚未開放」**，避免有人照著去接動作。要上線時先改圖再設區塊 |
