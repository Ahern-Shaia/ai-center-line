# 台灣福祉 × Ragic × LINE AI 智慧工廠戰情室 — 系統開發說明書 v0.7（完整版）

> 交付對象：技術長／工程團隊。
> **本版＝v0.6 + 缺陷修正 + 三項 P0 決策拍板，取代 v0.6。** v0.6 的「單一系統四層角色、tickets 通用事件表、human-in-the-loop 簽核」方向保留並精修；矛盾與缺口（雲端/地端、多模態選型、consultant 代簽、假名化/稽核、失效處理、成本、NFR、驗收）已補齊為可實作規格。
>
> **v0.7 相對 v0.6 的關鍵變更（changelog）**
> 1. 部署決策拍板：**只有服務部署在地端**；LLM 分析可用雲端，但一律吃「去識別後」內容（§三）。
> 2. 多模態逐場景選型（§三-B）：語音/知識庫索引留地端，其餘 Claude 分階。
> 3. `tickets.module_type` enum 與六群組對不齊 → 改以 `department_id` 為路由主鍵（§五、§六）。
> 4. 補第六個 Agent（人資總務），五 Agent → 六 Agent（§六、§十二）。
> 5. 新增個資治理/假名化/稽核/RLS（§七）、可靠性與失效處理（§十）、成本量化（§十三）、NFR（§十四）、驗收測試（§十五）、整合契約（§十六）。
> 6. consultant 代簽：**預設不可**，授權則強制標記＋稽核（§八）。
> 7. 全文範例假名化（原 v0.6 範例用真名，已移除）。

---

## 目錄
一、專案背景與商業目標　二、單一系統四層角色權限　三、部署與模型架構（含多模態選型）　四、整體系統架構　五、資料架構　六、六大群組對接與訊息路由　七、個資治理・假名化・資安　八、每日簽核防呆機制　九、戰情室前端規格與指標邏輯　十、可靠性與失效處理　十一、多模態 RAG 與 SFT　十二、後端 AI Agent System Prompt　十三、成本模型　十四、非功能需求　十五、驗收與測試計畫　十六、整合契約　十七、開發階段與時程　十八、待確認事項
...
**變現模式**：包裝為標準產品，多租戶架構，License 授權管理。**目前 20 家製造業客戶排隊**，系統從第一天即以多租戶設計，不可為單一客戶寫死。

**核心解題原則**：不改變員工既有 LINE 使用習慣，後端 AI 負責抓取、分析、結構化，Ragic 作為工廠萬用系統總台。

---

## 二、單一系統，四層角色權限

只有**一個戰情室應用**（一套前端、一套 API），依登入者 `role` 動態決定視圖與資料範圍；權限差異收斂在後端角色判斷與 DB 層過濾（§七 RLS），不做兩套系統。
...
**關鍵差異（`aiproot_admin` vs `consultant`）**：系統商員工預設看不到任何客戶營運內容（保護機密）；顧問因需實際協助導入，在被指派範圍內享有近 `tenant_admin` 的內容存取。差異源於業務角色本質——系統商賣工具、顧問做服務——並以 DB 層物理隔離落實（§七）。
...
**「資料不外流」的誠實定義（對委員的話術照此）**：*服務、資料庫、原始媒體、語音、知識庫索引皆在地端；送雲端的僅為去識別後的文本與已遮罩圖像——機敏原始資料與可識別個資不出場。* 不宣稱「完全不透過雲端 API」；此定義可驗（§十五 測試）。日後客戶若要求更純地端，可逐群組把雲端 Claude 換為地端 SFT 模型，架構不變。
...
| 6 | 技術研發・KM/法規 | 長文字 | Claude 長上下文摘要 ＋ BGE-M3 embedding（地端索引）＋ 工研院 RAG | 混合 | 知識庫索引地端不外流；生成用 Claude/工研院（§十六） |
...
tenant_vocabulary ( id PK, tenant_id FK, term, standard_meaning, category, usage_count, source enum[manual/correction], created_at, updated_at )
                 -- ‹v0.7› source：correction_log 可回饋詞庫
...
ragic_sync_outbox ( id PK, ticket_id FK, target enum[ragic/itri], payload, attempts int, last_error nullable, created_at, done_at nullable )  -- ‹v0.7› Outbox（§十）
```

### 5.5 稽核（‹v0.7› 新增，治理系統必要件）

```sql
audit_log ( id PK, actor_user_id FK, actor_role enum, action enum[view_tenant/view_ticket/sign_off/proxy_sign/export/assign_consultant/...],
...
### 7.3 保存期限（資料最小化）
`raw_messages.content` 原文保存 N 天（建議 30–90，租戶可設）後刪除原文、僅保留結構化 tickets 與去識別摘要；媒體設 TTL。由 `retention_policy` 控。
...
4. 負責人點確認 → 該批 `confirm_status=已簽核` → 入 `ragic_sync_outbox`（§十）。
...
**設計鐵律：畫面每一個數字都必須能回答「從哪張表算出來」，不接受純裝飾示意數字。** 本專案已有參考實作（`src/warroom/aggregate.ts`），由 tickets 實算出三環形指標。

| 區塊 | 呈現 | 計算邏輯 | 資料表 |
|---|---|---|---|
| 本日簽核完成率 | 儀表 | 已簽核群組 ÷ 6 | `tickets.confirm_status` by department |
| 六群組整體健康度 | 儀表 | 綠燈群組 ÷ 6（狀態機見 §十.4） | `tickets` + `raw_messages.created_at` |
| 今日 AI 高信心比例 | 儀表 | `confidence=high` ÷ 當日已分類 | `tickets.confidence`（**刻意不美化**） |
| 本月維修工單 | 數字卡 | 當月 `CRM_service_tickets` 計數 | `CRM_service_tickets` |
| 知識庫累積文件 | 數字卡 | RAG 索引文件累積數 | RAG 索引服務 |
...
> 前端美學另見設計文件；本案採 `civic-trust` profile（暖紙白×深松綠，serif 數字，每數字掛來源表），呼應上述鐵律。
...
**RAG**：原始文件 → 地端 Embedding（BGE-M3）/全文索引 → 向量/全文檢索 → Re-ranking → 提示工程 → LLM 精準回答（含來源標記）。知識庫索引留地端（§三）。

**SFT**：對地端開源模型（如 Llama 3 8B/11B）SFT，植入公司術語與標準答案，作為「逐群組把雲端 Claude 換地端」的長期選項。**對外呈現效能數字須標明「參考基準」或「本案場實測」，不可混用他案數字**（呼應 §九鐵律）。

---

## 十二、後端 AI Agent System Prompt（六 Agent，範例已假名化）
...
- **安全 hardening**：地端 OS/網路隔離、金鑰管理、最小權限、依 §七 稽核。

---

## 十五、驗收與測試計畫（‹v0.7› 新增；三大宣稱可當場驗）
...
## 十六、整合契約（‹v0.7› 收斂 TODO）

須向對方確認並寫入合約的介面契約：
- **工研院多模態 RAG**（窗口：資通所 鄧澤宇 Fisher TENG, 03-591-2068）：技轉或 API 串接？端點/認證/請求-回應格式/索引更新頻率/**IP 與資料歸屬**/SLA/地端可用性。
- **Ragic API**：讀主檔（employees/vehicles/work_orders）與寫（各群組目標表）的端點、認證、rate limit、寫入失敗語義（配合 §十 Outbox）、schema 版本控管。
...
| Day 4-5 | tickets + tenant_vocabulary + audit_log + 批次 cron（冪等） | 2 |
| Day 6-7 | 地端多模態（Whisper ASR / 遮罩 / NER / BGE-M3 索引） | 2 |
| Day 8-11 | Claude 分階 prompt（分類/抽取/grounding/vision，六 Agent） | 4 |
| Day 12-13 | 每日簽核面板 + 逾時警示 + Ragic Outbox 同步 + 工研院 RAG | 2 |
| Day 14-15 | 單一戰情室應用，四角色視圖（同套元件依 role 渲染） | 2 |
| Day 16 | 隔離/防幻覺/簽核 gate 驗收測試（§十五）+ 端對端 | 1 |
