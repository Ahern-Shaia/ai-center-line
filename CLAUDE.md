...
| **R16** | **前端設計鐵則**（戰情室後台開發時生效）：所有前端產出必過 `docs/frontend-design-principles.md` —— §A 普世核心一律適用、§C 設計流程迴圈動手前先跑；**§B 美學 profile 本專案鎖定 `modern-SaaS-craft`，不可換**。要換必須由人改本鐵則 + §B 並說明理由 |
