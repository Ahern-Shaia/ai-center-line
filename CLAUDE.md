...
| **R18** | **一律用台灣繁體中文（zh-TW）回覆** — 所有對使用者的回覆、狀態更新、任務／FMEA 報告、commit 訊息的說明文字皆用台灣慣用語繁體中文，不用簡體、不用英文段落。**例外**：程式碼識別字、指令、檔名、API 欄位、既定品牌／縮寫（Ragic / LINE / AIPROOT / AI / API 等）維持原文（見 AGENTS.md Naming）。技術名詞可中英並列，但敘述主體須為繁中 |
...
| **R16** | **前端設計鐵則**（戰情室後台開發時生效）：所有前端產出必過 `docs/frontend-design-principles.md` —— §A 普世核心一律適用（**含 §A1 動手前先鎖一個真實對標、§A5 先研究≥3競品再設計，不可略**；§A1 配色黑名單已刪＝不禁任何配色、改判執行）、§C 設計流程迴圈動手前先跑；**§B 美學 profile：戰情室 app 鎖定 `observability-light`**（2026-07-04 用戶裁定切換自 blueprint；定義見該檔 §B0-OL；對標 Datadog light / Grafana light / Metabase；淺色高密度儀表板；語意色 emerald/amber/rose；Inter+Noto Sans TC；mono 只在數字/代碼；無藍圖裝飾、無 §01 章節符、無角落量測記號、無網格背景；shell = 左 sidebar + 上 topbar + user menu）。**歷史 profile：**`blueprint`（藍圖繪圖美學，與企業 SaaS 慣例衝突，2026-07-04 廢止）、`civic-trust`（AI 預設群集）、`mission-control-dark`（暗色）。要換必須由人改本鐵則 + §B 並說明理由 |
...
| Ragic → LINE 通知 · 操作流程手冊 | `docs/sop/notify-ragic-line-操作流程.md` |
