...
-- 守住 v2「租戶不能自建/自升同級」。
--
-- ⚠️ permission_id 必須是 'resource:action' 字串（guard 與前端 perms.has() 都拿它比對）。
--    切勿用 gen_random_uuid() —— 那會讓 guard 永遠 deny、前端永遠不顯示（0056 修過這個坑）。
...
SELECT 'users:assign-role', 'users', 'assign-role', 'tenant', '調整成員角色（限 員工↔部門主管，碰不到高階帳號）'
 WHERE NOT EXISTS (
...
INSERT INTO permissions (permission_id, resource, action, scope, description)
SELECT 'users:delete-member', 'users', 'delete-member', 'tenant', '刪除自家成員（限 員工/部門主管，碰不到高階帳號）'
 WHERE NOT EXISTS (
