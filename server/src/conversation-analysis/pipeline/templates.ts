...
1. 報修派工單 = **冒號標籤表單**，連續多行。實際欄位（2026-08-03 prod 真實表單）：
   「來源：／公司：／客戶:／聯絡人:／電話:／車種:／車牌:／是否保固內:／地址:／狀況:」
   —— 各張不一定齊全，**有幾欄填幾欄**。這是客服貼出、描述「待處理」維修需求的單子（狀態在派工前）。
   ⚠️ **師傅的「今日進度回報／今日工作內容回報」不是報修單**，那種**一律略過、不要放進 service_intake**。
2. 訊息裡**沒有**報修派工單時，service_intake 回**空陣列**。不要把進度回報或閒聊硬塞進來。
   ⚠️ 只有空白範本（「請填寫下列資料」後面欄位全空）也**不算**一張單，不要抽。
3. 欄位對照表單原文：
   - **公司：或 客戶: → customer**（⚠️ 實際表單多用「公司：」；**兩者都要對到 customer**）
   - 聯絡人: → contact ｜ 電話: → phone（照抄完整號碼，系統會自動遮罩）
   - 車種＋車牌 → vehicle（合併原文，如「得利卡 BQZ-7806」）
   - 地址: → site（只取區域，如「新北市五股區」「南區」）｜ 狀況: → issue
   ⚠️ **多數報修單是個人車主、沒有公司** —— 這時 customer 就填 null，**不要把聯絡人姓名或車牌當成客戶**。
4. warranty 由「是否保固內」映射：是→「保內」、否→「保外」、未知／空白→null。
   這是**讀取表單明寫的欄位值**，不是推斷保固期 —— 沒有此欄就 null，仍禁止自行判斷。
5. status：**這種表單通常沒有狀態欄** —— 只有訊息明講（待派工／待聯絡客戶／已安排）才填，否則 null。
   ⚠️ 不要從「狀況」的內容自行推斷狀態。
