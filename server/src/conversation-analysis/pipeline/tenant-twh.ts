...
- facility_management 廠區設施：廠房與場地本身 —— 停車場、門禁、監視器、水電、
  網路佈線與機房、辦公空間整修、參訪場佈
...
7. **meeting（會議記錄）是「有主題就歸主題」的最後歸宿，不是優先分類**：
   ...
   **不可以因為它在會議上被提到就改歸 meeting**，否則要找維保時就找不到它了。
   ...
   以及會議本身的通知、議程、時間協調。
8. **maintenance／it_support／facility_management 三者的判別軸是「壞的是什麼」**：
   車輛、產線設備、工具治具 → maintenance；
   帳號權限與軟體操作（NAS／ERP／桌機） → it_support；
   **廠房與場地本身**（停車場、門禁、監視器、水電、網路佈線、空間整修） → facility_management。
   例：「停車場地面坑洞維修」不是 maintenance（它不是車輛也不是產線設備）→ facility_management；
       「監視器加裝」是實體設施 → facility_management，
       但「監視器的帳號登不進去」是軟體操作 → it_support。
...
8. due_at / due_text —— **這兩欄是給行事曆用的，填錯比留空更糟**：
   ...
   · ⚠️ **不可以臆測**。分界是「算得出唯一答案」還是「要用猜的」：
     · 「今天」「明天」「後天」—— 上面已經給了日期，算得出唯一答案，**可以**換算。
     · 「下週三」「月底前」「下個月」「9月初」—— 要用猜的，**不可以**換算：
       due_at 填 ""、due_text 保留原文（「下週三」），讓人自己判斷。
     （原本這條寫「絕對不可以自己換算」，但模型一直在換算「今天」而且換得對 ——
       規則寫得比實際嚴，只會讓後來的人以為那是 bug 去「修」掉一個好行為。）
   · ⚠️ 訊息在描述**已經做完的事**（不是未來的安排）時，也是兩欄都填 ""。
   · ⚠️⚠️ **一筆記錄只能有一個日期。一則訊息裡排了好幾天的事，就拆成好幾筆記錄。**
     排程類訊息（拖車、拍照、交車、進車、出貨…）常常一次講一整個月，例如
     「9/2 創奕拖車、9/4 拍照、9/4下午國捷拖車、9/7下午國捷拖車、9/10 交車」。
     這種**必須拆成 5 筆**，每筆一個 due_at、title 寫那一天要做的那件事、
     due_text 只寫那一筆自己的原文（「9/4下午」），**不要把整串日期抄進每一筆**。
     · 拆出來的每一筆都填一樣的 source_ids（都指回同一則訊息）。
     · 只有**明確講到日期**的才拆；「9月初（日期待確定）」這種沒有確定日期的
       不要拆出一筆，併在最相關的那筆裡或整筆 due 留空。
     · 這一條只管**未來的安排**。在講已經做完的事，不管提到幾個日期都不拆。
     為什麼要拆：使用者是按「今天」去看有什麼事。一筆記錄只放得下一個日期，
     沒拆的話 9/4、9/7、9/10 那幾天他會看到空的，而那些事**明明已經抽出來了**。
...
- 一則訊息排了多天，一天一筆。例：
  #7 [2026-09-01 10:12] 業助: 9月排程 9/2 創奕拖車、9/4 拍照、9/4下午國捷拖車、9/7下午國捷拖車
  → records 出**四筆**，全部 source_ids: [7]：
    · {title: "創奕拖車", due_at: "2026-09-02", due_text: "9/2"}
    · {title: "拍照",     due_at: "2026-09-04", due_text: "9/4"}
    · {title: "國捷拖車", due_at: "2026-09-04", due_text: "9/4下午"}
    · {title: "國捷拖車", due_at: "2026-09-07", due_text: "9/7下午"}
  ⚠️ 不可以只出一筆 due_at: "2026-09-02"、due_text 塞整串 —— 後面三天會查無此行程。`;
