...
/** 跨租戶的入口只有一種形狀：tenantId 從 query 進來 */
const TENANT_FROM_CLIENT = /@Query\(\s*["']tenantId["']/;
...
test("⭐⭐ 吃 @Query(\"tenantId\") 的端點必須有租戶邊界把關", () => {
  ...
  const bad = crossTenant.filter((h) => {
    // ✅ 用 resolveTenantId() 把關的 —— 它會擋掉非平台角色傳別家 id
    if (/resolveTenantId\(/.test(h.block)) return false;

    const m = h.block.match(/@Roles\(([^)]*)\)/);
    if (!m) return true;                                   // 既沒 @Roles 也沒 resolveTenantId
  ...
  assert.deepEqual(
    ...
    + "三選一：限 @Roles(aiproot_admin, consultant)、"
    + "用 resolveTenantId(user, tenantId)、或改成從 @CurrentUser 取 tenant_id。",
  );
});
