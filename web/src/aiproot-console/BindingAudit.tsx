...
export default function BindingAudit() {
  ...
  return (
    <div className="pane">
      ...
      {/* Unbound nudge alert */}
      {selectedTenantUnbound && selectedTenantUnbound.unboundCount > 0 && (
        <div className="dm-empty" style={{ background: "var(--warn-tint)", border: "1px solid #F5D5A6", textAlign: "left", padding: 14, marginBottom: 16 }}>
          <b style={{ color: "#7A4E1B" }}>⚠ 未綁定活躍者 · {selectedTenantUnbound.unboundCount} 位</b>
          <div style={{ marginTop: 6, fontSize: 12 }}>
            這些 LINE 用戶近 7 天在群組發訊 · 但未完成綁定 · 建議追人請他們加 bot 好友：
          </div>
          <div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", gap: 8 }}>
            {selectedTenantUnbound.top.slice(0, 5).map((u) => (
            ...
            {selectedTenantUnbound.top.length > 5 && (
              <span style={{ fontSize: 12, color: "var(--ink-3)" }}>+ {selectedTenantUnbound.top.length - 5} 位</span>
            )}
          </div>
        </div>
      )}
