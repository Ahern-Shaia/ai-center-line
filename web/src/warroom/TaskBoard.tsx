...
function TicketCard({ t, onOpen }: { t: WarroomKanbanTicket; onOpen: () => void }) {
  ...
  const confTag = t.confidence === "high" ? "高" : t.confidence === "medium" ? "中" : t.confidence === "low" ? "低" : "";
  const confClass = t.confidence === "high" ? "ok" : t.confidence === "medium" ? "warn" : t.confidence === "low" ? "danger" : "";
  return (
    <button className="kb-card" onClick={onOpen}>
      <div className="kb-card-summary">{t.summary}</div>
      <div className="kb-card-meta">
        {t.category && <span className="kb-tag">{t.category}</span>}
        {confTag && <span className={`tag ${confClass}`}>{confTag}</span>}
        {t.assigneeDisplayName && <span className="kb-assignee">👤 {t.assigneeDisplayName}</span>}
        {dueText && <span className="kb-due">📅 {dueText}</span>}
      </div>
      ...
      <aside className="drawer">
        ...
        <div className="drawer-body">
          ...
          <dl className="drawer-meta">
            {ticket.category && (<><dt>分類</dt><dd>{ticket.category}</dd></>)}
