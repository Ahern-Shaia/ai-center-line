...
⚠️⚠️ **這份是示範資料集。** 檔案自己在彙總表最後一列寫著：
   「本檔為模擬資料，僅日期 20260826 對應原始報表；其餘 29 日依工位特性、
     星期效應與設定之特殊事件日隨機產生」
   → 30 天裡**只有 1 天對應原始報表**。

   2026-08-28 用戶裁定（這兩頁要拿去給客戶展示）：
   **不要頂部的紅色「模擬資料」警示橫幅** →
   改成**頁尾註腳**，寫法對齊批次品質分頁既有的 `.bq-note`。
   ⛔ 是「改位置與語氣」不是「刪內容」：客戶當場問「這 30 天是實際量的嗎」時，
      頁面上必須答得出來。`meta.disclaimer` 與 `meta.realDay` 一律照原樣輸出。
